...
- 貸出・交換率を反映した参考期待値と追加投資リスクのリアルタイム表示

## 期待値表示について

表示金額は、実戦値やメーカー公表値ではなく、感情打ちを止める判断材料として設計した独自の簡易モデルによる参考値です。貸出単価を`1,000円 ÷ 貸出枚数`、交換単価を`100円 ÷ 交換枚数`として交換ギャップを反映しています。北斗・東京喰種は入力地点ごとの仮の期待収支テーブルを線形補間し、ジャグラーは次の500G、3枚掛けと仮定してREG確率・サンプルゲーム数から控えめな出玉率を推定します。ハマリゲーム数そのものを次ゲームの当選確率上昇には使用しません。

これらは設定、内部モード、有利区間、ホール状況などを完全には反映できません。プラス表示を含めて勝利や利益を保証せず、金額を賭ける根拠には使用しないでください。
